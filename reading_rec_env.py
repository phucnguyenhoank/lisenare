...
class ReadingRecEnvContinuous(gym.Env):
    ...
    def step(self, action: np.ndarray):
        """
        Action must be a real item embedding in the database.
        """
        self.step_count += 1

        # Normalize action
        item_emb_norm = normalize(action)

        # Find most similar item to the action
        sims = self.emb_normed @ item_emb_norm
        ...
        event_logits = np.arange(len(POSSIBLE_EVENTS), dtype=float) * (reward_base - 0.5) * self.prob_scale

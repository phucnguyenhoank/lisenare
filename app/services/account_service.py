...
def change_learner_account_password(
    ...
    account = session.exec(select(Account).where(Account.learner_id == learner_id)).first()
    if not account:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Account not found")
    if not security.verify_password(old_password, account.hashed_password):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="The old password is wrong")

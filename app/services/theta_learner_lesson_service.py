...
from app.services.history_answer_question_service import get_difficulty_and_respone
def get_theta_by_leaner_and_lesson(session: Session, learner_id, lesson_id):
    statement = select(ThetaLearnerLesson.theta).where(ThetaLearnerLesson.learner_id == learner_id and ThetaLearnerLesson.lesson_id == lesson_id)
...
def insert_or_update_theta(session: Session, lesson_id:int, learner_id: int):
    items, responses = get_difficulty_and_respone(session=session, learner_id=learner_id, lesson_id=lesson_id)
    ...
    try: 
        if existing:
            ...
            session.add(ThetaLearnerLesson(lesson_id=lesson_id, learner_id=learner_id, theta=theta))

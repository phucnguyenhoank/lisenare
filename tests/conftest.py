...
1. Pytest looks inside your test function and sees `client` in the parentheses.
2. Pytest pauses your test and jumps over to run the `client()` fixture first.
3. Inside the fixture, it opens the `TestClient(app)`.
4. The fixture hits the `yield client` line. 
   It pauses right there and hands the client over to your test.
5. Your test function runs from top to bottom:
   • It overrides the login dependency.
   • It sends the `GET` request to your database.
...
[ END ] Pytest prints a green checkmark in your terminal.
"""
